...
class QdrantUpserter:
    ...
    def _process_embedded_message(self, message_id, message_data):
        ...
        try:
            ...
            if operation == 'delete':
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        logger.info(
                            f"🗑️ [STAGE 2] Deleted from Qdrant: {doc_id} -> {point_uuid}")
                        break
                    except Exception as e:
                ...
                if len(vector) != expected_size:
                    logger.error(
                        f"❌ Wrong vector size for {doc_id}: got {
                            len(vector)}, expected {expected_size}")
                    return
                ...
                point = PointStruct(
                    ...
                    payload={
                        ...
                        # Full content
                        'content_full': message_data.get('content_full', ''),
                        # Short preview
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        logger.info(
                            f"✅ [STAGE 2] Upserted to Qdrant: {operation} - {doc_id} -> {point_uuid}")
                        break
                    except Exception as e:
                        if attempt == max_retries - 1:
                            raise
            ...
            logger.error(
                f"❌ Failed to process embedded message {message_id}: {e}")
    ...
    def start_consuming(self):
        ...
        logger.info(
            f"📤 Writing to: Qdrant collection '{
                self.collection_name}'")
    ...
    def handle_pending_messages(self):
        ...
        try:
            ...
            if pending:
                logger.info(
                    f"📋 Found {
                        len(pending)} pending embedded messages, processing...")
