        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:  # Last attempt
                    ...
                    logger.warning(
                        f"⚠️ Embedding attempt {
                            attempt +
                            1} failed, retrying in {wait_time}s: {e}")
        ...
        try:
            ...
            if operation == 'delete':
                ...
                if not text_to_embed.strip():
                    logger.warning(
                        f"⚠️ No text to embed for {doc_id}, skipping...")
                ...
                embedded_message = {
                    ...
                    # Store full content
                    'content_full': message_data.get('content', ''),
                    # Preview for quick display
                    'content_preview': message_data.get('content', '')[:500] if len(message_data.get('content', '')) > 500 else message_data.get('content', ''),
                ...
                logger.info(
                    f"✅ [STAGE 1→2] Generated embedding for {doc_id} (size: {
                        len(vector)})")
            ...
            logger.info(
                f"✅ [STAGE 1→2] Forwarded to embedded stream: {embedded_message_id}")
    ...
    def handle_pending_messages(self):
        ...
        try:
            ...
            if pending:
                logger.info(
                    f"📋 Found {
                        len(pending)} pending raw messages, processing...")
